...
def generate(paths, args):
    ...
    for path in paths:
        ...
        costs.append(travel / args.travel_speed + draw / args.draw_speed
                     + 2 * args.z_hop / args.z_speed + 2 * args.settle / 1000.0)
        pos = path[-1]
    ...
    lines = [
        ...
        f"travel={args.travel_speed}mm/s settle={args.settle:g}ms",
    ...
    # settle: dwell after every pen up/down so the spring module stops
    # bouncing before XY moves — the fix for stray marks between strokes
    settle = f"G4 P{args.settle:g}" if args.settle > 0 else None
...
def main():
    ...
    ap.add_argument("--settle", type=float, default=120.0,
                    help="dwell after every pen up/down, ms (default 120; 0 "
                         "disables). Increase to 150–250 if stray marks appear "
                         "between strokes")
